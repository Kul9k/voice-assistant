...
class AdvancedFeatures:
    def __init__(self, data_manager, voice_engine):
        ...
        try:
            ...
            self.yandex_music = YandexMusicController(voice_engine)
            print("✅ Яндекс.Музыка загружена")
        except ImportError:
    ...
    def handle_music_command(self, command):
        ...
        if not self.yandex_music:
            print("❌ Яндекс.Музыка не доступна")
            self.voice_engine.play_more_details()
            return False

        # Перенаправляем в основной обработчик
    ...
    def _handle_yandex_music(self, command):
        ...
        for word, action in quick_commands.items():
            if word in cmd_lower:
                return self.yandex_music.control_playback(action)

        # Команды воспроизведения
        if "включи" in cmd_lower:
            query = cmd_lower.replace("включи", "").strip()

            # Специальные команды
            if "радио" in query:
                radio_type = query.replace("радио", "").strip()
                if not radio_type:
                    radio_type = "стандартное"
                return self.yandex_music.play_radio(radio_type)

            # Обычный поиск
            return self.yandex_music.smart_play(query)

...
class AIAssistant:
    def __init__(self):
        ...
        try:
            self.yandex_music = YandexMusicController(self.voice_engine)
            print("✅ Яндекс.Музыка инициализирована")
        except ImportError:
            print("⚠️ Яндекс.Музыка не доступна")
            self.yandex_music = None
    ...
    def _handle_yandex_music(self, command):
        ...
        # (работает только если пользователь уже запустил музыку)

        if "пауза" in cmd_lower or "стоп" in cmd_lower:
            success = self.yandex_music.control_playback('pause')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "продолжи" in cmd_lower or "играй" in cmd_lower:
            success = self.yandex_music.control_playback('play')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "следующий" in cmd_lower or "дальше" in cmd_lower:
            success = self.yandex_music.control_playback('next')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "предыдущий" in cmd_lower or "назад" in cmd_lower:
            success = self.yandex_music.control_playback('prev')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "громче" in cmd_lower:
            success = self.yandex_music.control_playback('volume_up')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "тише" in cmd_lower:
            success = self.yandex_music.control_playback('volume_down')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "нравится" in cmd_lower or "лайк" in cmd_lower:
            success = self.yandex_music.control_playback('like')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "перемешать" in cmd_lower or "шафл" in cmd_lower:
            success = self.yandex_music.control_playback('shuffle')
            if success:
                self.voice_engine.play_random_success()
            return success

        elif "повтор" in cmd_lower:
            success = self.yandex_music.control_playback('repeat')
            if success:
                self.voice_engine.play_random_success()
            return success

        # КОМАНДЫ ПОИСКА И ОТКРЫТИЯ
        # (открывают Яндекс.Музыку, но не запускают автоматически)

        elif "включи" in cmd_lower or "поставь" in cmd_lower or "открой" in cmd_lower:
            # Извлекаем запрос
            if "включи" in cmd_lower:
                music_query = cmd_lower.split("включи")[-1].strip()
            elif "поставь" in cmd_lower:
                music_query = cmd_lower.split("поставь")[-1].strip()
            else:  # "открой"
                music_query = cmd_lower.split("открой")[-1].strip()

            # Проверяем специальные команды
            if "радио" in music_query:
                radio_type = music_query.replace("радио", "").strip()
                if not radio_type:
                    radio_type = "стандартное"
                success = self.yandex_music.open_radio(radio_type)
            elif "артист" in music_query:
                artist = music_query.replace("артиста", "").strip()
                success = self.yandex_music.play_artist(artist)
            else:
                # Умный поиск
                success = self.yandex_music.smart_search(music_query)

            if success:
                self.voice_engine.play_random_success()

                # Дополнительная голосовая инструкция
                print("\n💡 После выбора трека в браузере управляйте голосом:")
                print("   'пауза', 'продолжи', 'следующий', 'громче'")

            return success

        # НАСТРОЙКА АККАУНТА
        elif "настрой музыку" in cmd_lower or "привяжи аккаунт" in cmd_lower:
            print("\n🎵 Настройка Яндекс.Музыки...")
            success = self.yandex_music.setup_user_account()
            if success:
                self.voice_engine.play_random_success()
            return success

        # СПРАВКА
        elif "помощь музыка" in cmd_lower:
    ...
    def start_console_mode(self):
        ...
        print("\n🎵 МУЗЫКАЛЬНЫЕ КОМАНДЫ (Яндекс.Музыка):")
        print("  музыка включи [запрос] - поиск и воспроизведение")
        print("  музыка радио [тип] - включить радио")
        print("  музыка пауза - поставить на паузу")
        print("  музыка играй - продолжить")
        print("  музыка следующий - следующий трек")
        print("  музыка громче - увеличить громкость")
        print("  музыка нравится - добавить в 'Мне нравится'")
        print("  музыка статус - показать статус")
        print("\n🎵 МУЗЫКА (Яндекс.Музыка):")
        print("  музыка включи [запрос] - поиск (без автозапуска)")
        print("  музыка радио [тип] - открыть радиостанцию")
        print("  музыка артист [имя] - открыть артиста")
        print("  музыка пауза - пауза/продолжить")
        print("  музыка следующий - следующий трек")
        print("  музыка громче/тише - громкость")
        ...
        while True:
            try:
                ...
                if user_input.startswith('музыка '):
                    ...
                    # Обработка команд
                    if music_cmd.startswith('включи'):
                        query = music_cmd.replace('включи', '').strip()
                        if query:
                            self.yandex_music.smart_search(query)
                        else:
                            print("❌ Укажите что включить")

                    elif music_cmd.startswith('радио'):
                        radio_type = music_cmd.replace('радио', '').strip()
                        if not radio_type:
                            radio_type = "стандартное"
                        self.yandex_music.open_radio(radio_type)

                    elif music_cmd.startswith('артист'):
                        artist = music_cmd.replace('артист', '').strip()
                        if artist:
                            self.yandex_music.play_artist(artist)
                        else:
                            print("❌ Укажите имя артиста")

                    elif music_cmd in ['пауза', 'стоп']:
                        self.yandex_music.control_playback('pause')

                    elif music_cmd in ['играй', 'продолжи']:
                        self.yandex_music.control_playback('play')

                    elif music_cmd in ['следующий', 'дальше']:
                        self.yandex_music.control_playback('next')

                    elif music_cmd in ['предыдущий', 'назад']:
                        self.yandex_music.control_playback('prev')

                    elif music_cmd == 'громче':
                        self.yandex_music.control_playback('volume_up')

                    elif music_cmd == 'тише':
                        self.yandex_music.control_playback('volume_down')

                    elif music_cmd == 'нравится':
                        self.yandex_music.control_playback('like')

                    elif music_cmd == 'настройка':
                        self.yandex_music.setup_user_account()

                    elif music_cmd == 'помощь':
                        self.yandex_music.show_help()

                    elif music_cmd == 'статус':
                        status = self.yandex_music.get_status()
                        print(f"\n📊 Статус Яндекс.Музыки:")
                        for key, value in status.items():
                            print(f"   {key}: {value}")

                    else:
                        print("❌ Неизвестная музыкальная команда")
